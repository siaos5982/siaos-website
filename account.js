(() => {
  ...
  const renderReports = reports => {
    ...
    }).join('') : `<div class="account-empty"><h3>No purchased reports linked</h3><p>Paid reports use a secure purchase record and cannot be unlocked by unverified email or phone details.</p></div>`;
  };
  const renderOrders=orders=>{
    document.querySelector('#orderCount').textContent=orders.length;
    orderTarget.innerHTML=orders.length?orders.map(order=>{const items=Array.isArray(order.items)?order.items:[];const names=items.map(item=>`${item.name}${item.size?` · ${item.size}`:''}${item.quantity>1?` × ${item.quantity}`:''}`).join(', ');return `<article class="history-card order-history-card"><span class="history-type">Order ${escapeHtml(order.orderNumber||order.id)}</span><h3>${escapeHtml(names||'SIAOS product order')}</h3><p>Payment: ${escapeHtml(order.paymentStatus||'pending')} · Delivery: ${escapeHtml(order.status||'awaiting confirmation')}</p><time>${escapeHtml(formatDate(order.orderedAt))}</time>${order.trackingReference?`<span class="order-tracking">Tracking: ${escapeHtml(order.trackingReference)}</span>`:''}</article>`;}).join(''):`<div class="account-empty"><h3>No product orders linked</h3><p>Orders use secure payment records and cannot be unlocked by unverified email or phone details.</p><a class="btn fill" href="products.html">Explore Products</a></div>`;
  };
